...
export default (router: Router): void => {
  ...
  router.post('/stop', async (req, res) => {
    try {
      ...
      const responseRows = JSON.parse(response).rowCount
      if (responseRows === 0) {
        res.status(405).send('Project doest exist, could not stop')
      } else {
        if (responseValues.status === 'STOPPED') {
          res.status(405).send('Project already stopped')
        } else {
          stopRegister(
            responseValues.project_name,
            responseValues.start_time,
            responseValues.id
          )
        }
      }
      ...
      res
        .status(200)
        .send(`Project ${responseValues.project_name} Tracking STOPPED`)
    } catch (error) {
      res.status(400).send(JSON.stringify(error))
    }
  })
